...
- **Notificaciones recibidas con la app completamente cerrada**: al iniciar, `GooglePayListenerService.start()` hace un "drain" de las notificaciones actualmente activas (todavía visibles en la bandeja del sistema, vía `NotificationListenerService.getActiveNotifications()`) además de suscribirse al stream en vivo — esto cubre el caso de un pago hecho mientras la app estaba cerrada, siempre que la notificación siga visible cuando el usuario abre la app. Sigue existiendo una brecha real y no resuelta: si el usuario descarta la notificación de Google Wallet (o Android la limpia) antes de abrir la app, ese pago se pierde sin ningún registro ni forma de recuperarlo en este alcance.
